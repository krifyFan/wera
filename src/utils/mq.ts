...
export class MQ {
    ...
    //断开与MQ的连接
    closeMQ() {
        (this.client as Client).disconnect(() => {
    ...
    _onConnect(callback: any) {
        let _this = this;
        return function () {
            (_this.client as Client).subscribe(`/${_this.MQParams.connectType}/${_this.MQParams.connectName}`, callback.apply(_this))
        }
    }
    ...
    _InitMQ(callback: any) {
        ...
        // 获得Stomp client对象
        _this.client = Stomp.over((_this.ws as WebSocket));
        // SockJS does not support heart-beat: disable heart-beats
        _this.client.heartbeat.outgoing = 5000;
        _this.client.heartbeat.incoming = 5000;

        _this.client.connect(this.MQParams.login, this.MQParams.passcode, _this._onConnect(callback), _this._onError, '/');
    };
}
